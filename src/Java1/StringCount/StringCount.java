...
public class StringCount {
   public static void main(String[] args) {
        ...
        words = words.replace(",", "")        /// удаляем знаки препинания
                .replace(".", "")
                .replace("!", "")
                .replace("?", "")
                .replace("\"", "")
        ...
        String[] allWords = words.split(" ");         ////  создаём массив строк

        Map<String, Integer> map = new HashMap<>();
        for (int i = 0; i < allWords.length; i++) {                  /// считаем слова
            if (allWords[i] == null || allWords[i].trim().isEmpty()) {
                continue; // Don't use continue!
            }
            String w = allWords[i].toLowerCase();         /////// убираем заглавные буквы
            Integer count = map.get(w);             /// кладём в коллекцию
            if (count == null) {
                map.put(w, 1);
            } else {
                map.put(w, count + 1);
        ...
        System.out.println("Самое частое слово - '" + mostCommonWord + "'. Встретилось " +min + " раз.");
      }
    }
